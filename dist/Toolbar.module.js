...
function Button({ width, icon, hoverColor, backgroundColor }) {
    ...
    return (createElement("div", { onMouseEnter: handleMouseEnter, onMouseLeave: handleMouseExit, style: {
            width,
...
function ToolbarWindows({ color }) {
    return (createElement("div", { className: "windows-toolbar", style: { backgroundColor: color || "#2f343f" } },
        createElement("div", { className: "windows-toolbar-state-buttons" },
            createElement(Button, { icon: createElement(MinimizePageState, null) }),
            createElement(Button, { icon: createElement(NormalPageState, null) }),
            createElement(Button, { hoverColor: "#e81123", icon: createElement(ExitPageState, null) }))));
}
...
function Toolbar({ color, defaultOS, forcedOS }) {
    const selectedToolbar = (option) => {
        switch (option) {
            case "WIN":
                return createElement(ToolbarWindows, { color: color });
            case "MAC":
